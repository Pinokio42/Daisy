...
module.exports = {
  config: {
    ...
    run: async (bot, message, args) => {
        ...
        } else if (args.join(' ').toLocaleLowerCase() == 'common') {
            let sembed2 = new MessageEmbed()
                .setColor("GREEN")
                .setDescription(`❌ You don't have a common to sell`);

            let common = await db.fetch(`common_${user.id}`)

            if (common < 1) return message.channel.send(sembed2)

            db.fetch(`common_${user.id}`)
            db.subtract(`common_${user.id}`, 1)

            let sembed3 = new MessageEmbed()
                .setColor("GREEN")
                .setDescription(`✅ Sold a common For 1200 Coins`);
